...
def Analyze_Reddit_Posts(stocks, df, common_words):
    ...
    for stock in stocks.itertuples():
        sum, count, positive, positive_Upvotes, negative, negative_Upvotes, neutral, neutral_Upvotes = 0,0, 0, 0, 0, 0, 0, 0
        ...
        for item in df.itertuples():
            ...
            if stock_ in item.Title or stock_ in item.Content:
                ...
                if predict_sentiment(model, item.Content) >= 0.5:
                  positive += 1
                  positive_Upvotes += item.Upvotes
                elif predict_sentiment(model, item.Content) <= 0.2:
                    negative += 1
                    negative_Upvotes += item.Upvotes
                else:
                    neutral += 1
                    neutral_Upvotes += item.Upvotes
        ...
        if count > 0 and len(stock.Stock_Name) > 4 and stock_ not in common_words:
            result.append({'Stock_Name': stock.Stock_Name,
                           ...
                           'Upvotes' : sum,
                           'Positive' : positive,
                           'Positive_Upvotes': positive_Upvotes,
                           'Negative' : negative,
                           'Negative_Upvotes': negative_Upvotes,
                           'Neutral' : neutral,
                           'Neutral_Upvotes': neutral_Upvotes})
...
def Analyze_tweets(stocks, df, common_words):
    ...
    for stock in stocks.iterrows():
        sum, count, positive, positive_Retweets, negative, negative_Retweets, neutral, neutral_Retweets = 0,0, 0, 0, 0, 0, 0, 0
        ...
        for item in df.itertuples():
            if stock_ in item.text:
                ...
                if semt >= 0.5:
                    positive_Retweets += item.retweet_count
                    positive += 1
                elif semt <= 0.2 :
                    negative_Retweets += item.retweet_count
                    negative += 1
                else:
                    neutral_Retweets += item.retweet_count
                    neutral += 1
        ...
        if count > 0 and len(stock_) > 4 and stock_ not in common_words:
            result.append({'Stock_Name': stock_,
                           ...
                           'Total_Retweets': sum,
                           'Positive' : positive,
                           'Positive_Retweets' : positive_Retweets,
                           'Negative' : negative,
                           'Negative_Retweets' : negative_Retweets,
                           'Neutral' : neutral,
                           'Neutral_Retweets' : neutral_Retweets})
